...
variables = ["close","price_index"]
df5 = cumu_chg(df4,variables,"wk").sort_values(["cmc_url","date"])
...
df5["vol_mc"] = df5["volume"]/df5["market_cap"]    
df5["pos_address_share"] = df5["positive_address_count"]/(df5["positive_address_count"] + df5["zero_address_count"])
df5["zero_address_share"] = df5["zero_address_count"]/(df5["positive_address_count"] + df5["zero_address_count"])             

#%%
df5.to_csv("onchain_chg_bins.csv")   
...
for i in vars[3:]:
  ...
  Y.plot(title = i+"_excess return" +" simple average", ax=axs)
  plt.show()
  fig.savefig(i+"_excess return"+"_simple average")

# %% compute weighted average of each quintile (quintiles are based on week 52 of market_cap or another variable)

wk_num = 52
variable = "excess_return_cumu_chg_wk"   #"market_cap"
a, b, c, d, e = percentiles(df5,wk_num,variable)
more_var = ["cmc_url","weeks_listed","date"]
...
for i in vars[3:]:
  ...
  xlabel = groupvar
  ylabel = ""
  title = "weighted average "+i+ " by "+ variable + " quintiles"
  figname = "weighted average"+i+ "by"+ variable + "quintiles"
  leg=["quintile 1","quintile 2","quintile 3","quintile 4","quintile 5"]
